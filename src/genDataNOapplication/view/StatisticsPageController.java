...
public class StatisticsPageController {
	...
	private void loadStatistics(int comNum) {
		...
		// Chart for Profiles
		{
			...
			int profileCommunityAssaign[] = this.configuration.getProfileCommunityAssaignment();
	        ...
	        //for(int i = 0; i < profilesFrequencies.length; i++)
	           	//profileChartData.add(new PieChart.Data("C" + profileCommunityAssaign[i] + " P"+i, profilesFrequencies[i]));
	        for(int i = 0; i < profileCommunityAssaign.length; i++)
	           	profileChartData.add(new PieChart.Data("C" + i + " P"+profileCommunityAssaign[i], profilesFrequencies[profileCommunityAssaign[i]]));
	        ...
	        profileChart.setTitle("Community proportions / profile seed assignments");
		...
		{	        
            ...
            if(comNum == 10)
            	series1.setName("Attribute-value default");
            else
            	series1.setName("Profile seed");           	
			...
			{
				{
					String[][] columns= {
						{"Atribute","atribute",},
						{"Profile seed " + profileID,"profile"},
					};			
